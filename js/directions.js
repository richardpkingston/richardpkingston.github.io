...
function initialize() {
    ...
    var latlng = new google.maps.LatLng(53.601164, -1.789623);
    ...
    geocoder.geocode({
        ...
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            if (results[0]) {
                ...
                $('.autoLink span').html(address).parent().show().click(
                    function() {
    ...
    directionsService.route(request, function(response, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            if (status == 'ZERO_RESULTS') {
                alert(
                    'No route could be found between your starting point and the cricket club.'
                );
                ...
                alert(
                    'Your starting point of origin could not be geocoded.'
                );
            } else if (status == 'INVALID_REQUEST') {
                alert(
                    'The Directions Request provided was invalid.'
                );
            } else {
                alert(
                    "There was an unknown error in your request. Request status: \n\n" +
                    status);
            }
        }
    });
